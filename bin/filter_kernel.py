...
--cut=0.4       control the plotting vmin and vmax
...
"""
    option_dict = read_option(sys, help_string, 1, 7)
    ...
    cut = option_dict.setdefault('cut', '1.')
    ...
    if plot_flag == 'yes':
        if comp == 'ka':
            subprocess.call("plot_kernel.py --comp=ka --cut="+cut, shell=True)
        elif comp == 'kb':
            subprocess.call("plot_kernel.py --comp=kb --cut="+cut, shell=True)
